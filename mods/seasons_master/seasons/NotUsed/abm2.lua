...
local function Add_snow_above(pos, addBlock, replaceNods)
	
	local above = {x = pos.x, y = pos.y + 1, z = pos.z}
	local name = minetest.get_node(above).name
	if name ~= "air" and not replaceNods and name ~= "default:snow" then
		return
	end
	...
	--Raycasting to see if there is something above
	local raycast = minetest.raycast({x = pos.x, y=above.y + 25, z=pos.z}, {x = pos.x, y=above.y, z=pos.z},false, true)
...
local function FreezeSoil(pos)
	...
	local above = {x = pos.x, y = pos.y + 1, z = pos.z}
	if minetest.get_node_light(above) >= 14 and addsnowBlock then
		minetest.set_node(above, {name = "default:snow"})
	end
end
...
if seasons.season == "winter" then
	...
	local nextStromTime = math.random(120,300)
	local stimer = 0
	minetest.register_globalstep(function(dtime)
		stimer = stimer+dtime
