...
function carbone_mobs:register_mob(name, def)
	minetest.register_entity(name, {
		...
		set_velocity = function(self, v)
			...
			print("Y value: " .. self.object:getvelocity().y)
			if x < -214748 or x > 214748 or x == nan then
				x = 0
				print("Bad X Value")
			else
			...
			if yaw >= 0 then
				...
				--print("The yaw value was found to be nan! ")
				yaw = 0
			end
...
--			end
			...
			if self.state == "stand" then
				if math.random(1, 4) == 1 then
					...
					self.object:setyaw(yaw+((math.random(0,360)- 14.50)/180*math.pi))
				end
				self.set_velocity(self, 0)
