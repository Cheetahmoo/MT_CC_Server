...
function hb.register_hudbar(identifier, text_color, label, textures, default_start_value, default_start_max, default_start_hidden, format_string)
	...
	hudtable.add_all = function(player, hudtable, start_value, start_max, start_hidden)
		...
		if hb.settings.bar_type == "progress_bar" and identifier ~= "temp" then
		...
		if identifier == "temp"	then 
			ids.bar = player:hud_add({
				...
				number = barnumber,
				alignment = {x=-1,y=-1},
				offset = { x = offset.x + 19 , y = offset.y},
			})
			ids.bg = player:hud_add({
				hud_elem_type = "image",
				position = pos,
				scale = bgscale,
				text = "seasons_temp_hud_bg.png",
				alignment = {x=1,y=1},
				offset = { x = offset.x - 6 , y = offset.y - 9 },
			})
			ids.text = player:hud_add({
				hud_elem_type = "text",
				position = pos,
...
function hb.init_hudbar(player, identifier, start_value, start_max, start_hidden)
	...
	if identifier ~= "temp" then
		hb.hudtables[identifier].add_all(player, hudtable, nil, nil, start_hidden)
	else
	...
	if hudtable.hudstate[name].hidden == false then
		...
		if value_changed or max_changed then
			...
			if hb.settings.bar_type == "progress_bar" then
				...
				if new_text ~= hudtable.hudstate[name].text  and identifier ~= "temp" then
